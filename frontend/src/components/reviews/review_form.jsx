...
export default class ReviewForm extends Component {
  constructor(props){
    ...
    reviewDisplay(){
      console.log("hello")
      this.setState({formVisible: !this.state.formVisible}).then(()=>
      this.setState({
        rating: 
          {boozy: 0,
          sweet: 0,
  ...
  render() {
    ...
    if (this.state.formVisible){
      form = <form className="review-form " onSubmit={this.handleSubmit}>
          <div className="form-first-column ratings-container">
          <span className='rating-header'> How does this drink taste? </span> 
            {reviewCategories.map((category,i) =>(
              <div className='ratings-wrapper'>
                ...
                <input  
                    ...
                    min={0} 
                    max={10} 
                    value={(this.state.rating[category]) ? (this.state.rating[category]): null} 
                    onClick={e => this.handleSlide({category},e)}/>
                <div className="category-value-text">{this.state.rating[category]}</div>  
              </div>
            )
          )}
          </div>
          <div className="form-second-column">
            <label> Title</label>
            <input className="review-title-input" type="text" value={this.state.title} onChange={this.update('title')}/>
            <label> Body</label>
            <textarea className="review-body-input" type="text" value={this.state.body} onChange={this.update('body')}/>
            <button className='btn'><span className="">Submit Review</span></button>
          </div>
        </form>
