...
const FeedIndexItem = ({ item }) => {
    ...
    display = (
        <div>
            <div className="feed-item-top">
                <div>
                    <Link
                        to={`/users/item.reviewer._id`}
                        className="feed-item-handle"
                    >
                        {item.reviewer.handle}
                    </Link>
                    {" reviewed "}
                    <Link
                        className="feed-item-review-recipe-name"
            ...
            <span className="feed-recipe-rating">
                {[1, 2, 3, 4, 5].map((value) => (
                    <Star key={value} filled={value <= item.rating["rating"]} />
                ))}
            </span>
            <div className="feed-review-recipe-item">
                <img
                    className="feed-recipe-img"
                    src={item.recipe.imgUrl}
                    alt="Image"
                />
    ...
    return <div className="feed-index-item-container">{display}</div>;
};
